...
public class AES {
     ...
     */
    public static final String newline = System.lineSeparator(); //The newline for whatever system you choose to run in.
    ...
    private int mcHelper(int[][] arr, int i, int j)
    {
        int mcsum = 0;
        for (int k = 0; k < 4; k++) {
            int a = AES.galois[i][k];
            int b = arr[k][j];
            mcsum ^= mcCalc(a, b);
        }
        return mcsum;
    ...
    public void invMixColumns(int[][] arr) {
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                arr[i][j] = invMcHelper(tarr, i, j);
            }
        }
    }

    private int invMcHelper(int[][] arr, int i, int j) //Helper method for invMixColumns
    {
        int mcsum = 0;
        for (int k = 0; k < 4; k++) {
            int a = AES.invgalois[i][k];
            int b = arr[k][j];
            mcsum ^= invMcCalc(a, b);
        }
        return mcsum;
    ...
    public int[][] keySchedule(String key) {
        int binkeysize = key.length() * 4;  // Check the length in bits, not bytes
        System.out.println("Key size in bits: " + binkeysize);  // Should be 128, 192, or 256

        // Proceed with the rest of the key schedule
        int colsize = binkeysize + 48 - (32 * ((binkeysize / 64) - 2)); // Size of key scheduling
        int[][] keyMatrix = new int[4][colsize / 4]; // Create the matrix for key scheduling
        int rconpointer = 1;
        int[] t = new int[4];
    ...
    public void encryptState(int[][] state, int[][] keyMatrix) {
        int numRounds = 10; // Update for other key sizes
    ...
    public void decryptState(int[][] state, int[][] keyMatrix) {
        int numRounds = 10; // Update for other key sizes
        addRoundKey(state, subKey(keyMatrix, numRounds));
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                ...
                if (charIndex < input.length()) {
                    state[j][i] = input.charAt(charIndex);
                } else {
                    state[j][i] = 0; // Padding if needed
                }
            }
        }
        return state;
